...
let scrape = async() => {
        ...
        for(let p=1;p<=9000;p++){  // if p=0 , seebug will redirect to p=2839 instead
            for(let i=1; i<=20; i++){
                ...
                const result = await page.evaluate(async () => {
                    ...
                    try{
                        serial = document.querySelector('#j-vul-basic-info > div > div:nth-child(1) > dl:nth-child(1) > dd > a').innerText.trim().replace("\n"," ");
                    }catch(e){
                        serial = ''
                    }
                    try{
                        time = document.querySelector('#j-vul-basic-info > div > div:nth-child(1) > dl:nth-child(3) > dd').innerHTML.trim().replace("\n"," ");
                    }catch(e){
                        time = ''
                    }
                    try{
                        component = document.querySelector('#j-vul-basic-info > div > div:nth-child(2) > dl:nth-child(2) > dd > a').innerText.trim().replace("\n"," ");
                    }catch(e){
                        component = ''
                    }
                    try{
                        title = document.querySelector('#j-vul-title > span').innerText.trim().replace("\n"," ");
                    }catch(e){
                        title = ''
                    }
                    try{
                        type = document.querySelector('#j-vul-basic-info > div > div:nth-child(2) > dl:nth-child(1) > dd > a').innerText.trim().replace("\n"," ");
                       } catch(e){
                    ...
                    try{
                        cve = document.querySelector('#j-vul-basic-info > div > div:nth-child(3) > dl:nth-child(1) > dd > a').getAttribute('href').trim().replace("\n"," ");
                    }catch(e){
                        cve = "";
                    }
                    ...
                    if(affect_version == ''){
                        try{
                            affect_version = document.querySelector("#j-vul-basic-info > div > div:nth-child(2) > dl:nth-child(2) > dd").innerText.trim().replace("\n"," ");
                        }catch(e){
                            affect_version = '';
                        }
                    }
                    ...
                    try{
                        level = document.querySelector("#j-vul-basic-info > div > div:nth-child(1) > dl:nth-child(4) > dd > div").getAttribute("data-original-title").trim().replace("\n"," ");
                    }catch(e){
                        level = "";
                    }

                    if (serial == '' && time == '' && component == '' && title == '' && type == '' && cve == '' && affect_version && level == ''){
                        return null;
                    }
                ...
                await browser.close();    // if end , it will generate many chrome process

                if(result){
                    var versions = result.affect_version.replace("\n"," ")  //for python shell stdout.readline reason. do not contains \n
                    console.log(result.serial+"---" +result.title + "---"+ result.component+"---"+ result.level+ "---"+ versions +"---" + result.type + "---" +result.cve)
                    if(Object.keys(result).length>0){
                        //use mysql

                        var myDate = new Date();
                        var current_time = myDate.toLocaleString();
                        var  addSqlParams = [result.title, result.serial, result.time, result.component,result.cve,result.type, versions, result.level, current_time];
